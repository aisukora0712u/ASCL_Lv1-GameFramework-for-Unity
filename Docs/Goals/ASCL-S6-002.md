...
当前基线尚未完成本目标的正式验收。通过开发期热重载沉淀可跨项目复用、可观察和可验证的能力。
...
为“开发期热重载”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。

遵循 PLAN.md 的身份、生命周期、模拟、内容版本与后端能力边界。稳定接口破坏性变更提供迁移路径；未测试平台不标为支持。

## 验收条件

- [ ] 显示内容差异，先验证后提交有效版本，明确运行状态更新边界。 验证合法变更、缺失引用、无效类型和中途失败
- [ ] 明确运行状态与定义修改边界。
- [ ] 交付与验证证据已写入元数据 evidence，实际验证记录写入 log。

## 验证场景

显示内容差异，先验证后提交有效版本，明确运行状态更新边界。 验证合法变更、缺失引用、无效类型和中途失败；明确运行状态与定义修改边界。
