...
# ASCL-S6-003 · 层级运行数据容器

## 问题与预期收益

当前基线尚未完成本目标的正式验收。通过层级运行数据容器沉淀可跨项目复用、可观察和可验证的能力。
...
为“层级运行数据容器”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。
...
- [ ] 借鉴 DataNode 的路径式组织
- [ ] 声明类型、所有权、生命周期及持久化边界。 验证合法变更、缺失引用、无效类型和中途失败
- [ ] 明确运行状态与定义修改边界。
- [ ] 交付与验证证据已写入元数据 evidence，实际验证记录写入 log。

## 验证场景

借鉴 DataNode 的路径式组织；声明类型、所有权、生命周期及持久化边界。 验证合法变更、缺失引用、无效类型和中途失败；明确运行状态与定义修改边界。
