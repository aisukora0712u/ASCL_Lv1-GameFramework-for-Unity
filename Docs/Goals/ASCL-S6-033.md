...
# ASCL-S6-033 · 内容制作与验证工具

## 问题与预期收益

当前基线尚未完成本目标的正式验收。通过内容制作与验证工具沉淀可跨项目复用、可观察和可验证的能力。
...
为“内容制作与验证工具”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。

遵循 PLAN.md 的身份、生命周期、模拟、内容版本与后端能力边界。稳定接口破坏性变更提供迁移路径；未测试平台不标为支持。

## 验收条件

- [ ] 生成内容包、引用检查与可运行验证样例。 平台或内容后端失败可诊断
- [ ] 缺依赖、冲突和重复操作有一致结果
- [ ] 核对具体 SDK 再分发范围。
...
生成内容包、引用检查与可运行验证样例。 平台或内容后端失败可诊断；缺依赖、冲突和重复操作有一致结果；核对具体 SDK 再分发范围。
