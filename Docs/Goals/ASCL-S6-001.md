...
# ASCL-S6-001 · 服务器 ID 生成器

## 问题与预期收益

当前基线尚未完成本目标的正式验收。通过服务器 ID 生成器沉淀可跨项目复用、可观察和可验证的能力。
...
为“服务器 ID 生成器”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。
...
- [ ] 可选节点级生成器
- [ ] 验证节点配置、序列耗尽、时钟回拨、节点复用与重启。 覆盖时钟回拨、序列耗尽、重启和重复节点配置
- [ ] 记录身份保证的作用范围。
- [ ] 交付与验证证据已写入元数据 evidence，实际验证记录写入 log。

## 验证场景

可选节点级生成器；验证节点配置、序列耗尽、时钟回拨、节点复用与重启。 覆盖时钟回拨、序列耗尽、重启和重复节点配置；记录身份保证的作用范围。
