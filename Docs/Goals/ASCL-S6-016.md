...
当前基线尚未完成本目标的正式验收。通过程序化生成沉淀可跨项目复用、可观察和可验证的能力。
...
为“程序化生成”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。

遵循 PLAN.md 的身份、生命周期、模拟、内容版本与后端能力边界。稳定接口破坏性变更提供迁移路径；未测试平台不标为支持。

## 验收条件

- [ ] 种子、步骤、生成版本与结果验证。 记录种子/生成版本或分块身份
- [ ] 加载顺序变化、失败重入后世界引用有效。
- [ ] 交付与验证证据已写入元数据 evidence，实际验证记录写入 log。

## 验证场景

种子、步骤、生成版本与结果验证。 记录种子/生成版本或分块身份；加载顺序变化、失败重入后世界引用有效。
