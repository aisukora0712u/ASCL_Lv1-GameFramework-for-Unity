...
当前基线尚未完成本目标的正式验收。通过新手引导沉淀可跨项目复用、可观察和可验证的能力。
...
为“新手引导”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。

遵循 PLAN.md 的身份、生命周期、模拟、内容版本与后端能力边界。稳定接口破坏性变更提供迁移路径；未测试平台不标为支持。

## 验收条件

- [ ] 步骤条件、目标定位、跳过、重入和中断恢复。 分支、失败、跳过和恢复有可运行例子
- [ ] 奖励/成就不重复发放
- [ ] 复用条件与事件。
...
步骤条件、目标定位、跳过、重入和中断恢复。 分支、失败、跳过和恢复有可运行例子；奖励/成就不重复发放；复用条件与事件。
