...
当前基线尚未完成本目标的正式验收。通过文件访问层沉淀可跨项目复用、可观察和可验证的能力。
...
为“文件访问层”提供最小公共契约、适配边界、所有权和失败结果；复用已落地模块，不复制规则。
...
- [ ] 区分只读包、可写目录、容器和缓存
- [ ] 封装实际访问能力。 覆盖成功、取消、断点/失败恢复和版本不兼容
- [ ] 明确下载、安装与加载的不同结果。
- [ ] 交付与验证证据已写入元数据 evidence，实际验证记录写入 log。

## 验证场景

区分只读包、可写目录、容器和缓存；封装实际访问能力。 覆盖成功、取消、断点/失败恢复和版本不兼容；明确下载、安装与加载的不同结果。
