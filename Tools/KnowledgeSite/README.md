...
[实施目标专页](http://127.0.0.1:4317/#implementation)读取本仓库 `PLAN.md`、`GOALS.md`、`Docs/Plans/*.md` 与 `Docs/Goals/*.md`，提供阶段进度、目标筛选/详情、136 项初始能力覆盖、实施动态和项目驱动目录。首页与导航均有入口；准备工作独立于框架实施统计。

仓库 Markdown 是此功能的唯一权威来源。此页只读，不调用私人知识库的保存、扫描或备份流程；原有研究型“扩展路线图”继续使用本地库。状态与证据由后续实施任务更新。页面可见时每 30 秒刷新，获得焦点或手动刷新也会重新读取；文件无效或读取失败时停止展示完成率并报告诊断。
...
两个命令共用 `server/roadmap.ts`，均只读，不初始化私人库。`GET /api/project-roadmap` 返回文档、目标、汇总、覆盖和诊断；没有对应写入接口。完整格式与状态规则见仓库根 `GOALS.md`。平台支持、依赖满足和目标完成分别表达；无验收证据不得标记完成。
...
服务刚开放端口时，启动器会最多等待 3 秒让身份文件写入完成，再执行完整核验。若仍失败，错误会给出实际状态文件路径与错误代码：`ENOENT` 表示文件缺失，`EACCES` / `EPERM` 表示无法读取，`INVALID_JSON` 表示内容格式错误。启动与停止需使用同一 Windows 用户及相同的 `LOCALAPPDATA`、`ASCL_KNOWLEDGE_STATE` 环境；不要删除身份文件或用网页返回值重建身份来绕过核验。
