...
namespace ASCL.Gameplay.Actions {
    ...
    public readonly struct GameActionContext {
        public GameActionContext(int actionId, Combatant source, Combatant target, ActionTags tags = ActionTags.None, float magnitude = 0) {
    ...
    public readonly struct ActionResult {
        internal ActionResult(bool succeeded,string? error,int effects,int depth){Succeeded=succeeded;Error=error;EffectsExecuted=effects;MaximumDepth=depth;}
    ...
    public sealed class ActionPipeline {
        ...
        public async UniTask<ActionResult> ExecuteAsync(GameActionContext context,IReadOnlyList<IActionEffect> effects,CancellationToken cancellationToken=default) {
            if(effects==null)throw new ArgumentNullException(nameof(effects)); var budget=new Budget(MaximumDepth,MaximumEffects); var execution=new ActionExecution(this,budget);
            try { await execution.ExecuteAsync(context,effects,cancellationToken); return new ActionResult(true,null,budget.Effects,budget.MaxObservedDepth); }
            catch(ActionBudgetExceededException ex){return new ActionResult(false,ex.Message,budget.Effects,budget.MaxObservedDepth);}
        }
    ...
    public sealed class ActionExecution {
        ...
        internal ActionExecution(ActionPipeline pipeline,ActionPipeline.Budget budget){_pipeline=pipeline;_budget=budget;}
    ...
    public sealed class SetNumericBaseEffect : IActionEffect {
        public SetNumericBaseEffect(NumericKey key,float value){Key=key;Value=value;}
        public NumericKey Key{get;} public float Value{get;}
